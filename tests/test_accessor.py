...
from python.struct_access import (
    partial_struct,
    set_accessors,
    update_structs,
    Ptr,
...
def test_accessor_scalar():
    ...
    s = partial_struct(
        dump_struct_layout("struct x { int n; char a; char b; short sign; };", "x")["x"]
    )(MEM_BASE)
...
def test_accessor_array_scalar():
    ...
    s = partial_struct(
        dump_struct_layout("struct x { int n; short arr[4]; };", "x")["x"]
    )(MEM_BASE)
...
def test_accessor_array_struct():
    ...
    s = partial_struct(
        dump_struct_layout(
            "struct x { long n; struct { int n; short s; char c; } a[3]; };", "x"
        )["x"]
    )(MEM_BASE)
...
def test_accessor_pointer():
    ...
    s = partial_struct(dump_struct_layout("struct x { int *ptr; int x; };", "x")["x"])(
        MEM_BASE
    )
...
def test_accessor_invalid_pointer_deref():
    ...
    s = partial_struct(
        dump_struct_layout(
            "struct x { int (*fptr)(void); void *void_ptr; void *nullptr; };", "x"
        )["x"]
    )(MEM_BASE)
...
def test_accessor_pointer_to_array():
    ...
    s = partial_struct(dump_struct_layout("struct x { int (*aptr)[3]; };", "x")["x"])(
        MEM_BASE
    )
...
def test_accessor_pointer_struct():
    ...
    structs = dump_struct_layout(
        "struct y { int n; char c; }; struct x { struct y *yptr; };", None
    )
...
def test_accessor_set_scalar():
    ...
    s = partial_struct(
        dump_struct_layout("struct x { unsigned char a; short sign; };", "x")["x"]
    )(MEM_BASE)
...
def test_accessor_set_array():
    ...
    s = partial_struct(dump_struct_layout("struct x { char arr[5] };", "x")["x"])(
        MEM_BASE
    )
...
def test_accessor_set_invalid():
    ...
    s = partial_struct(
        dump_struct_layout("struct x { struct { int a; } b; int arr[5]; };", "x")["x"]
    )(MEM_BASE)
...
def test_accessor_sizeof():
    x = dump_struct_layout("struct x { int x; char c; int arr[3]; void *p; };", "x")[
        "x"
    ]
